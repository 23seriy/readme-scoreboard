...
describe("League badge", () => {
  const dataFor = {
    nba: { sport: "nba", alt: "NBA", data: { ...BASE_NBA_DATA, recentGames: [] } },
    mlb: { sport: "mlb", alt: "MLB", data: { ...BASE_MLB_DATA, recentGames: [] } },
    mls: { sport: "mls", alt: "MLS", data: { ...BASE_MLS_DATA, recentGames: [] } },
  };

  Object.values(dataFor).forEach(({ sport, alt, data }) => {
    it(`${sport} renders a league logo with a dark-theme source`, () => {
    ...
    it(`${sport} keeps the conference/division text on the same line`, () => {
      const out = render(sport, data);
      const badgeLine = out.split("\n").find((l) => l.includes("<picture>"));
      expect(badgeLine).toBeDefined();
      // The badge must be a prefix, not a line of its own.
      expect(badgeLine.replace(/<picture>.*<\/picture>\s*/, "").trim().length).toBeGreaterThan(0);
    });

    it(`${sport} pairs a light src with a dark srcset`, () => {
      const out = render(sport, data);
      expect(out).toMatch(/srcset="[^"]*500-dark[^"]*"/);
      expect(out).toMatch(/<img src="[^"]*500\/[^"]*"/);
    });
  });

  it("uses distinct logos per league", () => {
    const nbaOut = render("nba", dataFor.nba.data);
    const mlbOut = render("mlb", dataFor.mlb.data);
    expect(nbaOut).toContain("leagues/500/nba.png");
    expect(mlbOut).toContain("leagues/500/mlb.png");
    expect(nbaOut).not.toContain("mlb.png");
  });

  it("uses ESPN's soccer league ids for MLS and EPL", () => {
    expect(render("mls", dataFor.mls.data)).toContain("leaguelogos/soccer/500/19.png");
    const eplOut = render("epl", {
      team: { abbreviation: "LIV", full_name: "Liverpool", conference: "Premier League" },
      record: { wins: 1, losses: 1, draws: 1, season: 2025 },
      recentGames: [], emoji: "🔴", logoUrl: "x",
    });
    expect(eplOut).toContain("leaguelogos/soccer/500/23.png");
  });

  it("does not disturb the team logo that sits above it", () => {
    const out = render("nba", dataFor.nba.data);
    expect(out).toContain('align="right"');
    // Team logo first, league badge after the heading.
    expect(out.indexOf('align="right"')).toBeLessThan(out.indexOf("<picture>"));
  });
});
